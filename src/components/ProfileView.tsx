...
export function ProfileView({ profile, updateProfile, onStartChat, isGuestMode = false, onLogin }: ProfileViewProps) {
  ...
  const handleJurisdictionChange = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (isGuestMode) {
      toast.info('Войдите в систему для сохранения настроек');
      if (onLogin) onLogin();
      return;
    }
  ...
  const toggleConsent = async () => {
    if (isGuestMode) {
      toast.info('Войдите в систему для предоставления согласия');
      if (onLogin) onLogin();
      return;
    }
    try {
      await updateProfile({ hasConsent: Number(profile?.hasConsent || 0) > 0 ? "0" : "1" });
      toast.success('Статус согласия изменен');
    } catch (error) {
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
            ...
            <Card>
              ...
              <CardContent>
                <form onSubmit={handleJurisdictionChange} className="space-y-4">
                  ...
                  >
                    <option value="" disabled>Выберите страну/регион</option>
              ...
              <div className="flex flex-col md:flex-row gap-6 items-center justify-between">
                <div className="space-y-1 text-center md:text-left">
                  <div className="flex items-center gap-2 justify-center md:justify-start">
                    ...
                    <h3 className="font-bold">Согласие на обработку данных</h3>
                  </div>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              <div className="text-[10px] uppercase font-bold text-muted-foreground">Анализ в реальном времени</div>
            </CardHeader>
            <CardContent>
               ...
               {!isDiagComplete && (
                 <div className="p-6 border rounded-2xl bg-secondary/30 border-dashed flex flex-col items-center text-center space-y-3">
                   ...
                   <div>
                     <p className="font-bold">Анализ не завершен</p>
        ...
        <div className="space-y-6">
          <Card className="border-primary/20 bg-primary text-primary-foreground overflow-hidden relative shadow-xl shadow-primary/10">
            ...
            <CardHeader>
              <CardTitle className="text-lg">Умные сценарии</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4 relative z-10">
              ...
              <div className="space-y-3 pt-2">
                <ScenarioItem title="Консолидация долга" active={isDiagComplete} />
                <ScenarioItem title="Исправление кредита" active={isDiagComplete} />
                <ScenarioItem title="Проверка на банкротство" active={isDiagComplete} />
              </div>

              <Button onClick={onStartChat} className="w-full bg-primary-foreground text-primary hover:bg-secondary mt-4 group font-bold">
                {isDiagComplete ? 'Перейти к сценариям' : 'Создать сценарии'} <ArrowRight className="ml-2 h-4 w-4 group-hover:translate-x-1 transition-transform" />
              </Button>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                <ActivityItem label="Регистрация" time="Сегодня" />
                <ActivityItem label="Сессия начата" time="Сегодня" />
              </div>
            </CardContent>
          </Card>
        </div>
...
function JourneyStep({ label, status }: { label: string, status: 'completed' | 'in_progress' | 'pending' | 'locked' }) {
